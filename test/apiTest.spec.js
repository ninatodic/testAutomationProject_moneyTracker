...
const utils = require('../APITests/utils');
const apiHandler = require('../APITests/APIhandler');
const verify = require('../APITests/verifications');
...
describe('Test user route', () => {
  describe('Test  POST request /api/users', () => {
    ...
    before('Create test user', async () => {
      testUser = await utils.createRandomUser();
    });

    it('should register a user succesfully', async () => {
      //send request to register a user
      const response = await apiHandler.registerUser(testUser);
      verify.userRegistrationSuccess(response);
    });

    it('should not register a user with existing email in db and should return correct error message', async () => {
      const response = await apiHandler.registerUser(testUser);
      verify.userRegistrationFail_existingEmail(response);
    });
    ...
    it('should return correct error message for invalid name (empty string)', async () => {
      ...
      const response = await apiHandler.registerUser(testUser);
      verify.userRegistrationFail_noName(response);
    });
    ...
    it('should return correct error message for invalid mail', async () => {
      ...
      const response = await apiHandler.registerUser(testUser);
      verify.userRegistrationFail_invalidEmail(response);
    });
    ...
    it('should return correct error message for invalid password (length >= 6)', async () => {
      ...
      const response = await apiHandler.registerUser(testUser);
      verify.userRegistrationFail_invalidPass(response);
    });

    it('should return array with three errors when all emtpy fields are sent', async () => {
      testUser = {};

      const response = await apiHandler.registerUser(testUser);
      verify.userRegistrationFail_emptyFields(response);
    });
  });
});
